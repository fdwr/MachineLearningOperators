...
| Single axis element gather 1D input    | [PyTorch take](https://pytorch.org/docs/stable/generated/torch.take.html) | Same as above, but input tensors is flattened to 1D first.<br/><sup>`input.rank == 1 after flattening to 1D`<br/>`output.shape == indices.shape`<br/>`input.shape = [indexable dimension as 1D]`<br/>`indices.shape = [index dimensions...]`<br/>`axis = implicitly 0`<br/>`output.shape = [index dimensions...]`</sup>
